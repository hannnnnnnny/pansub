...
- Fullscreen support: PanSub subtitles and controls move into the fullscreen Panopto player
- Stable subtitle anchoring reduces per-line jumping when Panopto captions reflow
...
Current release version: `1.1.12`.
...
- 可拖动的悬浮快捷控制，支持紧凑模式、本次隐藏、按网站隐藏、全局关闭和重置位置
- 支持全屏：PanSub 字幕和控制按钮会自动进入 Panopto 全屏播放器
- 稳定字幕锚点，减少每句字幕更新时因为原生字幕重排产生的位置跳动
- 按实际播放器区域居中，打开目录侧栏或 DevTools 时也不会按整个浏览器窗口偏移
- 尽量阻止浏览器网页翻译改写 PanSub 字幕层和 Panopto 原生字幕节点
- 本地翻译缓存，重复字幕不需要反复请求
