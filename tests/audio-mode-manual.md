...
3. Confirm the disclosure says:
   ...
   > PanSub captures audio only from the tab you explicitly start. Chrome performs English speech recognition on this device. PanSub does not save the audio or a transcript. If Chrome's local Translator is unavailable, recognized text is sent to Google Translate only after you separately allow that fallback. Use PanSub only with course content you are authorised to access and for personal study only. Do not export or share course audio, captions, or translations.
...
8. Confirm Settings warns that machine translations may be inaccurate and the original lecture is authoritative.
9. Confirm the README and store copy identify PanSub as independent and unofficial.
...
2. Verify docked captions, fullscreen, manual subtitle position, subtitle lock, floating controls, page translation protection, glossary handling, and session translation cache still work.
3. Confirm repeated lines are reused while the page remains open, no `pansubCache` caption entries are written to `chrome.storage.local`, and reopening the page starts with an empty cache.
4. Verify the popup and Settings page in English and Chinese at desktop and narrow widths.
